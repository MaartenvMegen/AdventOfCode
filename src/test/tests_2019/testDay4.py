...
class Day4Tester(unittest.TestCase):
    def test_count_valid_passwords(self):
        ...
        total_valids = [number for number in range(range_min,range_max) if is_valid(str(number))]
        self.assertEqual(889, len(total_valids))

        total_valids = [number for number in range(range_min, range_max) if is_valid2(str(number))]
