    ...
    T: Copy,
{
    ...
    pub fn parse_to_grid(input: &str, parse_func: fn(char) -> T) -> Grid<T> {
        let mut grid = Grid::new();
        for (y, line) in input.split("\n").enumerate() {
            //println!("{:?}", line.clone().trim().chars().collect::<Vec<char>>());
            for (x, nr) in line.trim().chars().map(&parse_func).enumerate() {
    ...
    pub fn update_loc(&mut self, loc: Point, value: T) {
        self.map.insert(loc, value);
    }

    pub fn get_locations(&self) -> Vec<Point> {
        self.map.keys().copied().collect::<Vec<Point>>()
    }

    pub fn get_map(&self) -> &HashMap<Point, T> {
        &self.map
    }
    ...
    pub fn add_to_grid(&mut self, point: Point, value: T) {
    ...
    pub fn get_neighbour_key_value(&self, point: &Point) -> Vec<(Point, T)> {
        let offsets = [(1, 0), (-1, 0), (0, 1), (0, -1)];
